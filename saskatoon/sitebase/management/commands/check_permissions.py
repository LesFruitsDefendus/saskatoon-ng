...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for app_label, permissions in ALL_PERMISSIONS.items():
            ...
            app_models = {m._meta.model_name for m in app_config.get_models()}
        ...
        if missing:
            ok = False
            self.stderr.write(
                self.style.ERROR("The following models are missing from ALL_PERMISSIONS:")
            )
            for app_label, name in missing:
                self.stderr.write(f"  {app_label}.{name}")

        if not ok:
            self.stderr.write("Please update the corresponding permissions.py file(s).")
            raise SystemExit(1)
