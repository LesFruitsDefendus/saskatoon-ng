...
def available_equipment_points(
    ...
    try:
        ...
        is_active = Q(harvest__status__in=[
            Harvest.Status.SCHEDULED, Harvest.Status.READY
        ])
        ...
        """
        id = harvest.pk if harvest is not None else None
        filters = ~Q(harvest__pk=id) & (not_conflicting
                                        if id is not None else not_conflicting)
