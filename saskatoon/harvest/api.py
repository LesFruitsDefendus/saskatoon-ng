...
class HarvestViewset(LoginRequiredMixin, viewsets.ModelViewSet[Harvest]):
    ...
    filter_backends = [DjangoFilterBackend, SearchFilter]  # type: ignore  # mypy says it should be Union[type[BaseFilterBackend], type[BaseFilterProtocol[Harvest]]]
    ...
    def make_reservation(self, request, pk=None):
        ...
        try:
            ...
            messages.error(request, _("Harvest does not exist"))
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
class PropertyMapView(LoginRequiredMixin, generics.ListAPIView[Property]):
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response(
            {
                "data": response.data,
                'filter': get_filter_context(self, 'property'),
                'new': {
                    'url': reverse_lazy('property-create'),
                    'title': _("New Property"),
                },
